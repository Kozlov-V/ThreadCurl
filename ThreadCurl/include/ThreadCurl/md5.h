...
/*	本类使用方法示例
	CMD5 md5;
	char original_string[1024];
	sprintf(original_string, "222.20.54.67:8080");
	unsigned char result_data[16];

	md5.Process((unsigned char *)original_string, strlen(original_string), (unsigned char *)result_data);
	for(int index = 0; index < 16; index ++)
	{
		printf("%02X", result_data[index]);
	}
*/

#ifndef _TOM_TCAST_CHENMIN_MD5_H_
#define _TOM_TCAST_CHENMIN_MD5_H_
...
public:
	CMD5();
	//对一段数据进行MD5运算,返回16字节的结果,注意md5_result必须是一个16字节的数组指针
	unsigned char * Process(unsigned char * origin_data, unsigned int origin_length, unsigned char * md5_result);
...
#endif	//end _TOM_TCAST_CHENMIN_MD5_H_
