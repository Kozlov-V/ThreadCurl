...
unsigned char * CMD5::Process(unsigned char * origin_data, unsigned int origin_length, unsigned char * md5_result)
{
	Init();
	Update(origin_data, origin_length);
	Finalize();
	memcpy(md5_result, m_Digest, 16);
	return md5_result;
}
...
// the message digest and reset the context.
void CMD5::Finalize()
{
	uchar bits[8];
	uint4 index, padLen;
